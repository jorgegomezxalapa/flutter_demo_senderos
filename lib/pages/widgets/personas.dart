...
class _PersonasListState extends State<PersonasList> {
  ...
  Widget build(BuildContext context) {
    return _registros.length > 0
        ? ListView.builder(
            itemCount: _registros.length,
            itemBuilder: (context, index) {
              var nombreCompleto = _registros[index]
                      [DatabaseHelper.columnNombre] +
                  " " +
                  _registros[index][DatabaseHelper.columnPrimerApellido] +
                  " " +
                  _registros[index][DatabaseHelper.columnSegundoApellido];
              var datosComplementarios =
                  "Edad: ${_registros[index][DatabaseHelper.columnEdad]} |  Género: ${_registros[index][DatabaseHelper.columnGenero]} ";
              return ListTile(
