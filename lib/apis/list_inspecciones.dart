...
// A function that converts a response body into a List<Inspecciones>.
...
class ListInspeccionesService extends StatelessWidget {
  ListInspeccionesService({super.key});
  @override
...
class InspeccionesList extends StatelessWidget {
  const InspeccionesList({super.key, required this.inspecciones});
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Container(
        child: inspecciones.isEmpty?const Text("No hay formularios para mostrar."): //show message if there is no any student
        Column(  //or populate list to Column children if there is student data.
          children: inspecciones.map((i){
            return Card(
              child: ListTile(
                leading: const Icon(Icons.format_list_bulleted),
                title: Text(i.nombre_razon_social),
                subtitle: Text("Id:" + i.id.toString()),
                trailing: Wrap(children: [
                  IconButton(
                      onPressed: () async {
                        /*await downloadFormulario(i);
                        ...
                        print("despues del scafforld");*/
                      },

                      /*onPressed: (){
                      Navigator.push(context, MaterialPageRoute(builder: (BuildContext context){
                        return EditFormulario(id: i.id);
...
class InspeccionItem{
  ...
  InspeccionItem(this.fecha_inspeccion, this.nombre_razon_social);
}

//valores de la lista
final List<InspeccionItem> _inspeccionesList = [
  InspeccionItem("Inspección 1", 'domicilio'),
  InspeccionItem("Inspección 2", 'domicilio'),
  InspeccionItem("Inspección 3", 'domicilio'),
  InspeccionItem("Inspección 4", 'domicilio'),
  InspeccionItem("Inspección 5", 'domicilio'),
];
...
Widget _buildTable({bool sortAscending = true}){
  return DataTable(
    ...
    rows: _inspeccionesList.map<DataRow>((InspeccionItem inspeccionItem){
      return DataRow(
        //key: Key(inspeccionItem.name),
        cells:<DataCell>[
          DataCell(
              Text(inspeccionItem.nombre_razon_social)
          ),
          DataCell(
              Text(inspeccionItem.nombre_razon_social)
          ),
